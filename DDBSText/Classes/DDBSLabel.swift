 ...
 */
open class DDBSLabel: UIView, TextDebugTarget, TextAsyncLayerDelegate, NSSecureCoding {
    ...
    private lazy var _font = DDBSLabel._defaultFont
    /**
    ...
    @objc open var font: UIFont? {
        set {
            let f = newValue ?? DDBSLabel._defaultFont
            if _font == f { return }
            _font = f
    ...
    private func _updateLayout() {
        ...
        shrinkInnerLayout = DDBSLabel._shrinkLayout(with: innerLayout)
    }
    ...
    private func _showHighlight(animated: Bool) {
        ...
        if highlightLayout == nil {
            ...
            shrinkHighlightLayout = DDBSLabel._shrinkLayout(with: highlightLayout)
            if highlightLayout == nil {
                highlight = nil
            }
        }
    ...
    private func _updateOuterTextProperties() {
        ...
        _font = innerText.bs_font ?? DDBSLabel._defaultFont
...
//        weak var weakSelf = self
        ...
        task.display = { context, size, isCancelled in
            ...
            if layoutNeedUpdate_ {
                ...
                shrinkLayout_ = DDBSLabel._shrinkLayout(with: layout_)
                if isCancelled() {
                    return
                }
                layoutUpdated_ = true
        ...
        task.didDisplay = { layer, finished in
            ...
            guard let view = layer.delegate as? DDBSLabel else {
                return
            }
